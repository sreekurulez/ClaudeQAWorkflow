...
- `dummy-app/` source, to find what actually changed and what selectors it exposes
...
object `{"severity": "P0"|"P1"|"P2", "summary": "...", "caseId": "<optional>"}`, never a bare
string. No prose outside the JSON object, and no fenced fallback: emit only the raw JSON object
on stdout with nothing before or after it — the control plane parses stdout directly.
